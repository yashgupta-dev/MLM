			...
			<div class="page-content d-flex align-items-center justify-content-center">
                ...
                <div class="col-md-8 pl-md-0">
                  <div class="auth-form-wrapper px-4 py-5">
                    ...
                    <form class="signe_in_game">
                      <div class="form-group">
                        <label for="exampleInputEmail1">Userid</label>
                        <input type="text" name="username" class="form-control" id="exampleInputEmail1" placeholder="user id" value="<?php if (get_cookie('gameuser')) { echo get_cookie('gameuser'); } ?>">
                      </div>
                      <input type="hidden" name="<?= $this->security->get_csrf_token_name()?>" value="<?= $this->security->get_csrf_hash(); ?>">
                      <div class="form-group">
                        <div class="d-flex justify-content-between">
                            <div><label for="exampleInputPassword1">Password</label></div>
                            <div><a href="<?=base_url()?>customer/forget-password?q=app"><small class="text-muted">forget password ?</small></a></div>
                        </div>
                        <input type="password" name="password" class="form-control" id="exampleInputPassword1" autocomplete="current-password" placeholder="Password" value="<?php if (get_cookie('gamepass')) { echo get_cookie('gamepass'); } ?>">
                      </div>
                      <div class="mt-3">
                        <button type="button" id="sign_in_game_btn" class="btn btn-primary text-white mr-2 mb-2 mb-md-0">Sing in</button>
                       
                      </div>
                      <a href="<?=base_url()?>api/create-account" class="d-block mt-3 text-muted">Not a user? Sign up</a>
                    </form>
                  </div>
                </div>
              </div>
